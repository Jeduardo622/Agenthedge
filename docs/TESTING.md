...
- `poetry run python -m cli.paper_strategy_data_gap_review --gate-decision storage/audit/paper_strategy_tuning_gate_decision_<timestamp>.json --artifact-dir storage/audit` writes the paper-only missing-data clearance register for held tuning gates. Use repeated `--review-entry-json '{"reason":"missing_fundamentals","symbol":"QQQ","evidence_artifact":"storage/audit/<artifact>.json"}'` entries for per-blocker evidence, or `--acceptance-reason` only when a human reviewer explicitly accepts all missing fundamentals, sentiment, and catalyst research inputs as paper-only limitations.
