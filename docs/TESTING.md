...
- `poetry run python -m cli.provider_readiness --required-provider alpha_vantage --required-provider finnhub --required-provider fred --required-provider newsapi --artifact-dir storage/audit --raw` performs a secret-free offline credential-presence check against the current process environment and writes `provider_readiness_<timestamp>.json` for paper evidence. It does not load `.env`, bootstrap the runtime, contact providers, or print or persist credential values, and exits nonzero if any explicitly required provider is missing. Omit `--artifact-dir` to preserve stdout-only behavior.
...
- `poetry run python -m cli.paper_catalyst_session_closeout --artifact-dir storage/audit --session-date YYYY-MM-DD --runtime-audit storage/audit/runtime_events_paper-YYYYMMDD.jsonl --decision-artifact storage/audit/paper_decision_log_paper-YYYYMMDD_<timestamp>.json --prior-capture storage/audit/paper_strategy_tuning_capture_paper-YYYYMMDD_<timestamp>.json --health-artifact storage/audit/paper_broker_health_<timestamp>.json --health-history storage/audit/paper_broker_health_history_<timestamp>.json --audit-chain-report storage/audit/reports/audit_chain_report_ok_<timestamp>.json --observed-price 741.0 --observed-at 2026-06-29T20:00:00Z --movement-horizon same_session_close --provider-degradation accepted --provider-degradation-reason "<paper-only rationale>"` writes the audit-only closeout packet for a filled paper catalyst sample, including filled-order status, no matching open orders, provider-degradation acceptance, same-session movement, a refreshed lifecycle, and a refreshed strategy capture without changing strategy thresholds, broker wiring, runtime config, or live settings.
...
- `poetry run python -m cli.paper_decision_log --artifact-dir storage/audit --session-id paper-YYYYMMDD --decision hold --reason "<reason>" --artifact-ref <path>` records an audit-only operator decision against a paper session. In paper-broker mode, `AUDIT_LOG_PATH` now defaults to `storage/audit/runtime_events_paper-YYYYMMDD.jsonl` when unset. Add that Strategy Council artifact ref when it exists; the decision command extracts `quant_consensus` signal snapshots and emits a companion `paper_strategy_tuning_capture_paper-YYYYMMDD_<timestamp>.json` before `cli.paper_strategy_tuning_report` runs. Use `--strategy-signal-json`, `--expected-movement`, `--actual-movement`, `--movement-horizon`, `--rejected-trade-json`, `--hit-rate`, and `--catalyst-json` only for override or backfill evidence.
- `poetry run python -m cli.paper_review_board --artifact-dir storage/audit --min-stable-sessions 5` writes the read-only daily paper review board JSON and Markdown report, including the multi-day stability window, `sessions_shortfall`/`stability_blocker` details for clean session-count shortfalls, and reviewer packet evidence links.
...
- `poetry run python -m cli.paper_live_readiness_workbench build --artifact-dir storage/audit --stability-window 5` writes the human review workbench packet and supervised live-dry-run bridge plan without broker mutation or live enablement; if stability is blocked only by too few clean closed sessions, the unresolved question names the shortfall explicitly.
- `poetry run python -m cli.paper_stability_evidence_chain --artifact-dir storage/audit --session-date 2026-06-24 --generated-at 2026-06-24T23:59:00+00:00 --min-stable-sessions 3` runs the audit-only June 24 third stability session paper-reporting chain and writes linked health history/status, lifecycle, decision, review-board, live-readiness, workbench, and chain-summary artifacts without broker, scheduler, runtime config, or live-trading mutation.
- `poetry run python -m cli.paper_strategy_tuning_evidence_chain --artifact-dir storage/audit --session-date 2026-06-24 --generated-at 2026-06-24T23:59:00+00:00 --start-date 2026-06-22 --min-sessions 3` runs the audit-only June 24 post-session tuning evidence chain and writes linked decision-log, strategy-capture or existing backfill reuse, paper strategy tuning report, and chain-summary artifacts without broker, scheduler, runtime config, strategy behavior, or live-trading mutation.
- `poetry run python -m cli.paper_catalyst_postmortem --artifact-dir storage/audit --session-date 2026-06-24 --symbol SPY --catalyst-id "Investor day"` writes the paper-only catalyst postmortem for the June 24 SPY miss from existing decision-log, strategy tuning capture, tuning report, and runtime audit artifacts. It explains expected return, actual movement, risk/compliance context, and catalyst-quality takeaway without changing strategy thresholds, broker wiring, runtime config, or live settings.
- `poetry run python -m cli.paper_strategy_tuning_capture --artifact-dir storage/audit --session-id paper-YYYYMMDD --decision-artifact storage/audit/paper_decision_log_paper-YYYYMMDD_<timestamp>.json --actual-movement 0.008 --movement-horizon next_session_close --from-decision-capture` records review-horizon actual movement while preserving the decision-time agent signal snapshot, expected return, exposure, drawdown, and catalyst attribution for the tuning report without changing strategy behavior.
- `poetry run python -m cli.paper_strategy_tuning_report --artifact-dir storage/audit --start-date YYYY-MM-DD --end-date YYYY-MM-DD` writes the paper-only strategy tuning report from closed paper-session artifacts, including daily decision questions, risk/compliance blocks, post-decision outcomes, and missing strategy-quality evidence.
- `poetry run python -m cli.paper_strategy_tuning_gate --report storage/audit/paper_strategy_tuning_report_20260624T185531Z.json --artifact-dir storage/audit --catalyst-postmortem storage/audit/paper_catalyst_postmortem_paper-20260624_<timestamp>.json --data-gap-review storage/audit/paper_strategy_data_gap_review_<timestamp>.json --min-sessions-reviewed 3 --min-catalyst-hit-rate 0.5 --max-catalyst-direction-misses 0 --max-consensus-threshold-misses 0 --max-data-gap-blockers 0` writes the paper-only tuning gate decision for June 22-24, 2026 with keep/adjust/hold recommendations for catalyst quality, consensus misses, and data-gap blockers. It can attach the SPY `Investor day` postmortem as catalyst-quality context, remains audit-only, and does not change strategy behavior, execution thresholds, broker wiring, or live settings.
- `poetry run python -m cli.paper_strategy_data_gap_evidence --artifact-dir storage/audit --reason missing_fundamentals --symbol QQQ --session-id paper-20260624 --source manual_provider_review --field ProfitMargin=0.21 --field PERatio=31.2` records a structured paper-only evidence artifact that can be referenced by `cli.paper_strategy_data_gap_review --review-entry-json`.
- `poetry run python -m cli.paper_strategy_data_gap_review --gate-decision storage/audit/paper_strategy_tuning_gate_decision_<timestamp>.json --artifact-dir storage/audit --evidence-artifact storage/audit/<artifact>.json --accept-blocker "missing_news_sentiment:QQQ:paper-20260624=<paper-only rationale>"` writes the paper-only missing-data clearance register for held tuning gates. Use repeated `--evidence-artifact` entries for ready evidence artifacts and repeated `--accept-blocker reason:symbol:session_id=acceptance_reason` entries for per-blocker paper-only acceptance; use `--acceptance-reason` only when a human reviewer explicitly accepts all missing fundamentals, sentiment, and catalyst research inputs as paper-only limitations.
