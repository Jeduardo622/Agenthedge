...
- `poetry run python -m cli.paper_strategy_tuning_capture --artifact-dir storage/audit --session-id paper-YYYYMMDD --signal-json '{"agent":"quant","strategy":"catalyst","symbol":"SPY","direction":"buy","confidence":0.72}' --expected-movement 0.01 --actual-movement 0.008 --movement-horizon next_session_close` records paper-only strategy signal, rejected-trade, movement, exposure, drawdown, hit-rate, and catalyst-attribution inputs for the tuning report without changing strategy behavior.
