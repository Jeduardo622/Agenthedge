...
- `poetry run python -m cli.paper_rollout_packet --artifact-dir storage/audit --profile config/promotion-gates/paper_rollout.json` runs the release check and writes copyable `paper_rollout_packet_<timestamp>.md` and `.json` artifacts for PR/release notes. Pass `--rehearsal-artifact <path>` to build the packet from existing paper proof without placing a new canary order.
- Paper rollout operator checklist: `docs/OPS_RUNBOOK.md` section "Paper Rollout Release Checklist".
...
- **Fixture-backed smoke:** Run catalyst experiments without network access by setting `EXPERIMENTAL_STRATEGIES=catalyst` and `CATALYST_RESEARCH_INPUT_PATH=tests/fixtures/research_inputs/catalyst_calendar_spy.json`, then running `poetry run python -m cli.backtest --symbol SPY --start 2026-06-12 --end 2026-06-13 --capital 100000 --price-fixture tests/fixtures/backtest/catalyst_spy_prices.json --gate-profile config/promotion-gates/catalyst_fixture_experiment.json --storage-dir .cache/catalyst-fixture-smoke`. The gate profile writes `promotion_report.json` next to `result.json` and evaluates fixture, catalyst, strategy, trade, and stale-check evidence in the same run.
- **Promotion gate reader:** Compare a generated promotion report against explicit review thresholds without enabling any strategy: `poetry run python -m cli.promotion_gate --report .cache/catalyst-fixture-smoke/<run_id>/promotion_report.json --profile config/promotion-gates/catalyst_fixture_experiment.json`.
